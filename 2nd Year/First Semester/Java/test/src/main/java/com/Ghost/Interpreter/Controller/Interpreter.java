...
public class Interpreter {
    ...
    public ArrayList<Pair<Integer, IValue>> get_latches_all()
    {
        if(this.programStates.size() == 0)
            return new ArrayList<Pair<Integer, IValue>>();

        ArrayList<Pair<Integer, IValue>> result = new ArrayList<Pair<Integer, IValue>>();
        for(Integer key : this.programStates.get(0).get_latch_table().all().keySet())
            result.add(new Pair<Integer, IValue>(key, this.programStates.get(0).get_latch_table().get(key)));
