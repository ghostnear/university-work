...
public class AllocateStatement implements IStatement {
    ...
    public Dictionary<String, IType> typeCheck(Dictionary<String, IType> typeEnvironment) throws InterpreterException {
        IType typeVariable = typeEnvironment.get(this.name);
        IType typeExpression = this.expression.typeCheck(typeEnvironment);

        if(!(typeVariable.equal(new RefferenceType(typeExpression))))
            throw new CheckingException("AllocateStatement: Type of variable and type of expression do not match!");
