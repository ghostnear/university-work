...
SELECT * FROM Manga WHERE MangaType = 'manga'
UNION
SELECT * FROM Manga WHERE MangaType = 'manhwa';

SELECT * FROM Manga WHERE MangaType = 'manga' OR MangaType = 'manhwa';

-- b) 2 queries with the intersection operation; use INTERSECT [ALL] and AND;
SELECT * FROM Manga WHERE MangaType = 'manga'
INTERSECT
SELECT * FROM Manga WHERE PhysicallyOwned = 1;

SELECT * FROM Manga WHERE MangaType = 'manga' AND PhysicallyOwned = 1;
...
SELECT * FROM Manga WHERE MangaType = 'manga' AND NOT PhysicallyOwned = 1;
...
SELECT * FROM Manga WHERE Manga.ID IN (SELECT MangaID FROM MangasToGenres WHERE GenreID = 'Action');
SELECT * FROM Manga WHERE Manga.ID IN (
    SELECT MangaID FROM MangasToGenres WHERE GenreID IN (
        SELECT GenreID FROM MangasToGenres WHERE MangaID = 1
    )
)

-- f) 2 queries with the EXISTS operator and a subquery in the WHERE clause;
SELECT * FROM Manga WHERE EXISTS (SELECT * FROM Manga WHERE MangaType = 'manhwa');
SELECT * FROM Manga WHERE EXISTS (
    SELECT * FROM Manga WHERE MangaType = 'manhwa' AND PhysicallyOwned = 1
)
...
SELECT MangaType, COUNT(*) FROM Manga GROUP BY MangaType HAVING COUNT(*) > 2;
SELECT MangaType, COUNT(*) FROM Manga GROUP BY MangaType HAVING COUNT(*) > (SELECT COUNT(*) FROM Manga WHERE MangaType = 'manga');
SELECT MangaType, COUNT(*) FROM Manga GROUP BY MangaType HAVING COUNT(*) > (SELECT COUNT(*) FROM Manga WHERE MangaType = 'manga' AND PhysicallyOwned = 1);

-- i) 4 queries using ANY and ALL to introduce a subquery in the WHERE clause (2 queries per operator);
SELECT * FROM Manga WHERE TotalChapters > ANY (SELECT TotalChapters FROM Manga WHERE MangaType = 'manga');
SELECT * FROM Manga WHERE TotalChapters > ALL (SELECT TotalChapters FROM Manga WHERE MangaType = 'manga');

SELECT * FROM Manga WHERE TotalChapters > ANY (SELECT TotalChapters FROM Manga WHERE MangaType = 'manga' AND PhysicallyOwned = 1);
SELECT * FROM Manga WHERE TotalChapters > ALL (SELECT TotalChapters FROM Manga WHERE MangaType = 'manga' AND PhysicallyOwned = 1);

-- rewrite 2 of them with aggregation operators, and the other 2 with IN / [NOT] IN.
SELECT * FROM Manga WHERE TotalChapters > (SELECT MIN(TotalChapters) FROM Manga WHERE MangaType = 'manga');
SELECT * FROM Manga WHERE TotalChapters > (SELECT MAX(TotalChapters) FROM Manga WHERE MangaType = 'manga');

-- TODO: fix this
SELECT * FROM Manga WHERE TotalChapters IN (SELECT TotalChapters FROM Manga WHERE MangaType = 'manga' AND PhysicallyOwned = 1);
SELECT * FROM Manga WHERE TotalChapters NOT IN (SELECT TotalChapters FROM Manga WHERE MangaType = 'manga' AND PhysicallyOwned = 1);
