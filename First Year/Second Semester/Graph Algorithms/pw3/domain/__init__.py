...
class Graph:
    ...
    """
        :returns the shortest path between the vertexes as an array of vertices.
    """
    def shortest_path(self, startVertex : int, endVertex : int):
        # Invalid vertexes.
        if not self.is_vertex(startVertex) or not self.is_vertex(endVertex):
            raise Exception()
        ...
        queue = [[endVertex]]
        visited = [False] * self.get_vertices_count()
        visited[endVertex] = True

        while(len(queue) > 0):
            # Get the current considered best path.
            currentPath = queue.pop(0)

            # Go through all the neighbours of the last vertex in the path.
            for vertex in self.parse_inbound_neighbours(currentPath[-1]):
                if not visited[vertex]:
                    visited[vertex] = True
                    finalPath = [*currentPath, vertex]
                    if vertex == startVertex:
                        finalPath.reverse()
                        return finalPath
                    queue.append(finalPath)
                    
        return []
    
    """
        :returns the lowest cost path between the vertexes as an array of vertices.
    """
    def _reconstruct_cheapest_path(self, startVertex : int, endVertex : int, costs : list):
        # Reconstruct the path.
        path = [endVertex]
        while path[-1] != startVertex:
            # Get the lowest path cost neighbour.
            minCost = float('inf')
            minVertex = None
    ...
    """
        :returns an array that contains the cost and the lowest cost path between the vertexes as an array of vertices.
    """
    def cheapest_path(self, startVertex: int, endVertex: int):
        # Invalid vertexes.
        if not self.is_vertex(startVertex) or not self.is_vertex(endVertex):
            raise Exception()
        ...
        costs = [float('inf')] * (self.get_vertices_count() + 1)
        found = [False] * self.get_vertices_count()

        # The cost of the start vertex to all the vertexes is the cost of the edge.
        for vertex in self.parse_vertices():
            try:
                costs[vertex] = self.get_cost(startVertex, vertex)
            except:
                pass
        ...
        for vertex in self.parse_vertices():
            ...
            if minPosition > -1:
                ...
                for neighbour in self.parse_outbound_neighbours(minPosition):
                    costs[neighbour] = min(costs[neighbour], costs[minPosition] + self.get_cost(minPosition, neighbour))

        return [costs[endVertex], self._reconstruct_cheapest_path(startVertex, endVertex, costs)]
