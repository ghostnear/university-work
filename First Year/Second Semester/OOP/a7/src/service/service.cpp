...
void Service::export_and_open_list()
{
    if(userIndex == std::size_t(-1))
        return;
    ...
    {
        std::ofstream fout("user_list.csv");

        fout << "Title,Description,Link,Date,Time\n";
        for(auto data : get_user_events())
            fout << data.get_title() << "," << data.get_description() << "," << data.get_link() << "," << data.get_date().get_day() << ":" << data.get_date().get_month() << ":" << data.get_date().get_year() << "," << data.get_time().get_hours() << ":" << data.get_time().get_minutes() << ":" << data.get_time().get_seconds() << "\n";

        fout.close();

        system("notepad user_list.csv");
    }

    if(outputType == ExportType::HTML)
    {
        std::ofstream fout("user_list.html");

        fout << "<!DOCTYPE html>\n";
        fout << "<html>\n";
        fout << "<head>\n";
        fout << "<title>User List</title>\n";
        fout << "</head>\n";
        fout << "<body>\n";
        fout << "<table border=\"1\">\n";
        fout << "<tr>\n";
        ...
        {
            fout << "<tr>\n";
            fout << "<td>" << data.get_title() << "</td>\n";
            fout << "<td>" << data.get_description() << "</td>\n";
            fout << "<td><a target=_blank href=\"" << data.get_link() << "\">Open the event link.</a></td>\n";
            fout << "<td>" << data.get_date().get_day() << ":" << data.get_date().get_month() << ":" << data.get_date().get_year() << "</td>\n";
            fout << "<td>" << data.get_time().get_hours() << ":" << data.get_time().get_minutes() << ":" << data.get_time().get_seconds() << "</td>\n";
            fout << "</tr>\n";
        }
