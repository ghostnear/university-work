...
public class GUIView extends IView {
    ...
    TableView<Pair<Integer, IValue>> latchTable;
    ...
    private void populateView()
    {
        ...
        // Update latch table.
        TableColumn<Pair<Integer, IValue>, Integer> latchAddressColumn = new TableColumn<Pair<Integer, IValue>, Integer>("Location");
        latchAddressColumn.setCellValueFactory(p -> new SimpleIntegerProperty(p.getValue().getKey()).asObject());
        latchAddressColumn.setPrefWidth(100);

        TableColumn<Pair<Integer, IValue>, String> latchValueColumn = new TableColumn<Pair<Integer, IValue>, String>("Value");
        latchValueColumn.setCellValueFactory(p -> new SimpleStringProperty(p.getValue().getValue().toString()));
        latchValueColumn.setPrefWidth(100);

        latchTable.getColumns().clear();
        latchTable.getColumns().addAll(latchAddressColumn, latchValueColumn);

        latchTable.editableProperty().set(false);
        latchTable.getItems().clear();

        for(Pair<Integer, IValue> entry : interpreter.get_latches_all())
            latchTable.getItems().add(entry);

        latchTable.refresh();

        // Log the current state.
        interpreter.set_log_file("log.txt");
    }
        ...
        for(int index = 0; index < programs.size(); index++)
        {
            ...
            button.onMouseClickedProperty().setValue((event) -> {
                try {
                    ...
                    window.setScene(new Scene(createProfilerComponents()));
    ...
    private Parent createProfilerComponents()
    {
        ...
        latchTable = new TableView<Pair<Integer, IValue>>();
        ...
        container.add(new VBox(new Text("Latches:"), latchTable), 3, 2);
